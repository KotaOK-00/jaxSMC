r"""
Adaptive waste-free SMC with an uncoupled pCNL proposal on a Gaussian target,
at fixed dimension. The MH proposal parameters are (\rho, \tau):

    q(y | x) = N(\mu + \rho (x - \mu) + (1 - \rho) \lambda_t C \grad \ell(x), \tau^2 C),

where \ell is the log-likelihood, so that (1 - \rho) \lambda_t C \grad \ell is the
Langevin drift for the tempered potential -\lambda_t \ell. The ESJD criteria is
computed on a 2d (\rho, \tau) grid; \tau^2 + \rho^2 = 1 is the pCN frontier (at
\tau^2 = 1 - \rho^2 and zero drift the proposal preserves N(\mu, C)).
...
from adaptive_smc.proposals.pCNL import build_build_ARLW
...
def experiment_uncoupled_pcnl(config, keys):
    dim = config.get('dim')
    ...
    config.update(
        {"optimization_method": optimization_method_str, "params_optimization_method": params_optimization_method,
         "proposal": "build_build_ARLW",
         "rho_grid": rho_grid, "tau_grid": tau_grid,
         "tempering_sequence": my_tempering_sequence,
         "init_param": init_param})

    my_proposal = build_build_ARLW(jnp.zeros(dim), jnp.eye(dim))
...
if __name__ == "__main__":
    yaml_file = os.path.join(HERE, "exp_pcnl_gaussian.yaml")
    ...
    for name_of_my_config, config in y_config.items():
        if config.get('run', True):
            ...
            for keys in all_keys:
                experiment_uncoupled_pcnl(config, keys)
